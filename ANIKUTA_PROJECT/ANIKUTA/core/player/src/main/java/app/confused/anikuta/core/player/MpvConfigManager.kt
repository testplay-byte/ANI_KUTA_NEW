...
object MpvConfigManager {
    ...
    /**
     * MPV config directory name under `context.filesDir`. Kept as a public
     * constant so other components (e.g. `AnikutaMPVView` initOptions,
     * asset-copy routines) can reference the same directory without a
     * hard dependency on a `PlayerActivity` host.
     */
