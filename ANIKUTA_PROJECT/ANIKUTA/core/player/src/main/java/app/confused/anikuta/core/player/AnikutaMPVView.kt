 ...
 *
 * DI note: This view is inflated from XML (by Compose `AndroidView` →
 * `LayoutInflater.inflate(R.layout.mpv_view, ...)`), so it CANNOT use Koin
 * constructor injection. Instead, the host (Activity / screen-level Composable)
 * must populate [Companion.playerPreferences] from a Koin-scoped instance
 * BEFORE the view is inflated. Typically:
 *
 *   AnikutaMPVView.playerPreferences = koinGet<PlayerPreferences>()
 *   AndroidView(factory = { ctx -> LayoutInflater.from(ctx)
 *       .inflate(R.layout.mpv_view, null) as AnikutaMPVView })
 *
 * The companion is `lateinit var` so a missing init fails loudly on first
 * property access rather than silently falling back to defaults.
 *
...
class AnikutaMPVView(
    ...
    fun loadTracks(): Pair<List<VideoTrack>, List<VideoTrack>> {
        ...
        try {
            ...
            for (i in 0 until count) {
                ...
                //
                // NOTE: The same formatting logic is mirrored in
                // `subtitles/SubtitleTrackFormatter.kt` for reuse from non-view
                // callers (e.g. tests). The inlined copy below is preserved
                // verbatim from the OLD player to keep the documented L6 fix
                // intact. A future refactor may swap this call site to
                // `SubtitleTrackFormatter.formatTrackName(id, title, lang)`.
     ...
     *    (done in the host's initMpvView() — see MpvLifecycleController TODO
     *    in the new project). Setting it via `setOptionString` at init is
     *    silently ignored. `font-dir` is not an option aniyomi uses at all.
     *  - verbose `msg-level` → reverted to `all=warn` (matches aniyomi).
     *
     * The actual subtitle-rendering fix lives in the host's copyAssets()
     * (subfont.ttf now copied to the config-dir ROOT, not fonts/) — see
     * PLAYER_REDO_PLAN.md §1 for the full root-cause analysis. The asset
     * lives at `core/player/src/main/assets/subfont.ttf` in the new project.
     */
    override fun initOptions(vo: String) {
        ...
        // again, change this to "all=v" and the logLvl in the host initMpvView
        ...
        // root by the host's copyAssets(). Required for HTTPS subtitle
        ...
        // runtime option set by the host initMpvView().
    ...
    companion object {
        /**
         * Set by the host (Activity / screen-level Composable) from a
         * Koin-resolved [PlayerPreferences] BEFORE this view is inflated.
         * See class KDoc for the inflation-time DI pattern.
         */
        @JvmStatic
        lateinit var playerPreferences: PlayerPreferences

        /**
         * MPV config directory name (relative to `context.filesDir`). Matches
         * [MpvConfigManager.MPV_DIR]; mirrored here so [initOptions] doesn't
         * need an import just for one path string.
         */
